...
def see_tasks():
    if tasks:
        ...
        for idx, task in enumerate(tasks, start=1):
            print(
                f"{idx}. Task Title: {task['Task Title']}, Task Description: {task['Task Description']}"
            )
    else:
...
def update_tasks():
    ...
    if tasks:
        try:
            tasks_index = (
                int(input("Provide the Index of your task to be updated: ")) - 1
            )
            if 0 <= tasks_index < len(tasks):
                new_task_title = input(
                    "Provide your new title or (Press Enter to keep current title): "
                )
                new_task_description = input(
                    "Provide your new description or (Press Enter to keep current description): "
                )
                if new_task_title:
...
def delete_tasks():
    ...
    if tasks:
        try:
            tasks_index = (
                int(input("Provide the Index of the task to be deleted: ")) - 1
            )
            if 0 <= tasks_index < len(tasks):
